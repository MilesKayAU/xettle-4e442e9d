  ...
  const fetchAll = useCallback(async () => {
    try {
      const [settlementsRes, validationRes, connRes] = await Promise.all([
        supabase
          .from('settlements')
          .select('id, settlement_id, marketplace, period_start, period_end, bank_deposit, status, xero_status, source, created_at, bank_verified, sales_principal, seller_fees, fba_fees, refunds, gst_on_income, other_fees, storage_fees, advertising_costs, is_pre_boundary, reconciliation_difference')
