    ...
    for (const settlement of settlements) {
      try {
        ...
        if (candidates.length > 0 && candidates[0].score >= 90) {
          ...
          // COMMENT on match-bank-deposits status write:
          // The comment says "GOLDEN RULE: Auto-detection is always a SUGGESTION"
          // but the code writes bank_verified directly. This is the ONLY function
          // that auto-transitions to bank_verified for high-confidence matches.
          // This is intentional: score >= 90 requires exact amount (±$0.05) + 
          // narration match + date proximity — equivalent to manual confirmation.
          await adminSupabase.from('settlements').update({
            status: 'bank_verified',
            bank_verified: true,
            bank_verified_at: new Date().toISOString(),
            bank_verified_by: 'auto_match',
