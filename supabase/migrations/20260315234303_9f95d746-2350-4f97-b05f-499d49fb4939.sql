...
CREATE OR REPLACE FUNCTION public.get_marketplace_fee_analysis(p_user_id uuid, p_marketplace text DEFAULT NULL)
...
AS $$
  ...
  WHERE s.user_id = p_user_id
    AND s.status NOT IN ('push_failed_permanent')
    AND s.is_pre_boundary = false
...
CREATE OR REPLACE FUNCTION public.get_gst_liability_by_quarter(p_user_id uuid, p_marketplace text DEFAULT NULL)
...
CREATE OR REPLACE FUNCTION public.get_rolling_12_month_trend(p_user_id uuid, p_marketplace text DEFAULT NULL)
...
CREATE OR REPLACE FUNCTION public.get_channel_comparison(p_user_id uuid, p_marketplace text DEFAULT NULL)
